  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Branch performance, approvals and audit trail</p>
      </div>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {(loading ? [...Array(3)] : branches).map((b, i) => (
          <div key={b?.id ?? i} className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            {b ? (
              <>
                ...
                <dl className="space-y-2 text-sm">
                  <div className="flex justify-between"><dt className="text-gray-500">Deposits</dt><dd className="font-medium text-gray-900">{formatCurrency(b.total_deposits)}</dd></div>
                  <div className="flex justify-between"><dt className="text-gray-500">Accounts</dt><dd className="font-medium text-gray-900">{b.total_accounts}</dd></div>
                  <div className="flex justify-between"><dt className="text-gray-500">Customers</dt><dd className="font-medium text-gray-900">{b.total_customers}</dd></div>
                  <div className="flex justify-between"><dt className="text-gray-500">Active loans</dt><dd className="font-medium text-gray-900">{b.active_loans}</dd></div>
                </dl>
              </>
            ) : (
